...
function attachPriceTooltipPositioning(){
  const wraps = document.querySelectorAll("[data-tooltip]");
  wraps.forEach(wrap => {
    const box = wrap.querySelector(".price-tooltip-box");
    const trigger = wrap.querySelector(".price-tooltip-trigger");
    if(!box || !trigger) return;

    // reset style biar fresh
    box.style.left = "";
    box.style.right = "";
    box.style.top = "";
    box.style.bottom = "";
    ...
    function reposition(){
      // tampilkan dulu biar bisa dihitung ukurannya
      ...
      // default: di kiri harga, sedikit naik
      let placeLeft = true;
      let placeAbove = true;

      // kalau posisi kiri bikin kepotong layar kiri → pindah ke kanan
      if (wrapRect.left - boxRect.width < 10) {
        placeLeft = false;
      }

      // kalau posisi atas bikin kepotong layar atas → pindah ke bawah
      if (wrapRect.top - boxRect.height < 10) {
        placeAbove = false;
      }
    ...
    // kalau layar berubah ukuran, tooltip menyesuaikan
    window.addEventListener("resize", () => {
      if(box.classList.contains("show")) reposition();
    });
  });
}
